...
export default async function BuyerFormPage({
  params,
}: {
  ...
  if (UUID_RE.test(param)) {
    ...
    if (data) {
      ...
      if (hist) {
        ...
        redirect(`/forms/buyer/${agent.vanity_slug ?? agent.id}`);
      }
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f8fafc", padding: "24px 16px 48px" }}>
      <div style={{ maxWidth: 680, margin: "0 auto" }}>
        ...
        <FormRenderer formType="off_market_buyer" agentSlug={resolvedAgentId} />
      </div>
    </main>
  );
}
