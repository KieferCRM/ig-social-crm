...
export default async function ContactFormPage({
  ...
  if (UUID_RE.test(param)) {
    ...
    if (error) {
      ...
      if (data.vanity_slug) redirect(`/forms/contact/${data.vanity_slug}`);
    ...
    if (data) {
      ...
      if (hist) {
        ...
        redirect(`/forms/contact/${agent.vanity_slug ?? agent.id}`);
      }
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f8fafc", padding: "24px 16px 48px" }}>
      <div style={{ maxWidth: 680, margin: "0 auto" }}>
        ...
        <FormRenderer formType="contact" agentSlug={resolvedAgentId} source="contact_form" />
      </div>
    </main>
  );
}
