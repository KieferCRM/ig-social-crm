...
export default async function SmartFormPage({
  params,
}: {
  ...
  if (UUID_RE.test(param)) {
    ...
    if (error) {
      ...
      if (data.vanity_slug) redirect(`/forms/${data.vanity_slug}`);
    ...
    if (data) {
      ...
      if (hist) {
        ...
        redirect(`/forms/${agent.vanity_slug ?? agent.id}`);
      }
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f8fafc", padding: "24px 16px 48px" }}>
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        <SmartForm agentSlug={resolvedAgentId} />
      </div>
    </main>
  );
}
