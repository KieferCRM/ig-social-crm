    ...
    >
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {task.deal && (
            <Link
              href={isOffMarketAccount ? "/app/pipeline" : "/app/deals"}
...
export default function PrioritiesPage({ isOffMarketAccount = false }: { isOffMarketAccount?: boolean }) {
  ...
  return (
    <main className="crm-page crm-page-wide crm-stack-12">
      ...
      <section className="crm-card crm-section-card">
        <div className="crm-page-header">
          <div className="crm-page-header-main">
            <p className="crm-page-kicker">{isOffMarketAccount ? "Off-Market" : "Follow-Up Coordinator"}</p>
            <h1 className="crm-page-title">{isOffMarketAccount ? "Deal task queue" : "Follow-up queue"}</h1>
          ...
          <div className="crm-page-actions">
            ...
            <Link href={isOffMarketAccount ? "/app/pipeline" : "/app/deals"} className="crm-btn crm-btn-secondary">
              {isOffMarketAccount ? "Open pipeline" : "View Pipeline"}
            </Link>
          </div>
        </div>
      </section>
      ...
      >
        ...
        >
          {dueNowTasks.map((t) => (
            <TaskCard key={t.id} task={t} onDone={(id) => void markTaskDone(id)} onEdit={setEditingTask} marking={markingTask === t.id} isOffMarketAccount={isOffMarketAccount} />
          ))}
        ...
        {/* Column 2: Update This Deal / Stale Clients */}
        <Column
          title={isOffMarketAccount ? "Update This Deal" : "Stale Clients"}
          count={staleDeals.length}
          chipClass="crm-chip crm-chip-warn"
          empty={isOffMarketAccount ? "No stale deals right now." : "No stale clients right now."}
        >
          {staleDeals.map((deal) => (
            <div
              key={deal.id}
            ...
            >
              <div style={{ fontWeight: 600, fontSize: 13, color: "var(--ink-body)" }}>
                {deal.property_address ?? (isOffMarketAccount ? "Deal needs context" : "Client needs follow-up")}
              </div>
              ...
              <div style={{ fontSize: 12, color: "var(--ink-muted)", lineHeight: 1.5 }}>
                {isOffMarketAccount
                  ...
                  : "This client hasn't heard from you in a while. A quick check-in keeps deals from going cold."}
              </div>
              <Link href={isOffMarketAccount ? "/app/pipeline" : "/app/deals"} style={{ fontSize: 12, color: "var(--ink-primary)", textDecoration: "none" }}>
                {isOffMarketAccount ? "Open pipeline →" : "View in Pipeline →"}
              </Link>
            </div>
          ))}
        </Column>
